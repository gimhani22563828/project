...
exports.generateReport = async (req, res) => {
  try {
    ...
    const incomePercentage = ((incomeTotal / (incomeTotal + Math.abs(expenseTotal))) * 100).toFixed(2);
    const expensePercentage = ((expenseTotal / (incomeTotal + Math.abs(expenseTotal))) * 100).toFixed(2);
    reportText += `Income: ${'='.repeat(Math.round(incomePercentage / 10))} (${incomePercentage}%)\n`;
    reportText += `Expenses: ${'='.repeat(Math.round(expensePercentage / 10))} (${expensePercentage}%)\n`;

    // Check if there's any expense category breakdown
    ...
    if (categories.length > 0) {
      ...
      categories.forEach((category) => {
        reportText += `${category._id}: $${category.total.toFixed(2)}\n`;
      });
    }

    // Send report as plain text
